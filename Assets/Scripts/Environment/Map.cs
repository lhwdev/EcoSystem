...
public class Map {
	...
	public Map(Environment environment, int size, int regionSize) {
		...
		for (int i = 0; i < visibleRegions.Count; i++) {
			...
			for (int j = 0; j < map[regionCoord.x, regionCoord.y].Count; j++) {
				...
				if (sqrDst < sqrViewDst) {
					if (EnvironmentUtility.TileIsVisibile(environment, origin.x, origin.y, entity.coord.x, entity.coord.y)) {
